...
#### Если указывать номер AS не по умолчанию, то будет рассинхрон между bgp peer и bgp instance, из-за чего связность работать не будет.
#### Также стоит добавить, что необходимо на каждом устройстве в топологии указать routing bgp network с минимальной маской, обхватываюшей весь диапазон интерфейсов роутеров
